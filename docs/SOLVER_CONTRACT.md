...
- Items `deadlineReached: true` means the time budget ended the search. `capped: true` with `deadlineReached: false` means the search converged and stopped improving instead, and returns the same plan at any larger budget — the size of the remaining gap does not distinguish the two.
