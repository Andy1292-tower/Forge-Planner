...
Both are free by contract. A filler may consume only what its phase leaves unused after both its own consumption and the rate its own demand must be met at, so no busy line moves and every demanded material still lands no later than it did: the phase's own duration can only stay the same or fall, and it falls only where the search had been cut short holding a plan that parked a line. A fill is discarded if the filled phase no longer replays. Neither pass will craft a material carrying an external pre-produced Bits obligation, whose fixed point the phase has already closed; banking is further limited to direct project costs, since held stock does not remove a Set & forget feeder job. Ordering is settled before any fill, so no fill can reshuffle the queue it banks for.
