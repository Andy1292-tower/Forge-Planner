# Daily Items and Credits Cache Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Reuse identical Max Items and complete Max Credits solves for 24 hours, retain an explicit fresh-solve path, and remove the browser UI suite from required CI and the repository.

**Architecture:** `js/solve-service.js` owns a small versioned, mode-specific cache policy beside its existing request authority, so Worker and synchronous-fallback responses share one safe storage path. `js/results.js` passes a one-shot `forceFresh` request from Resimulate and renders mode-correct cache-hit status. Playwright is removed; focused Node lifecycle and build contracts remain.
...
- Cache Max Items and complete Max Credits comparisons for exactly 24 hours per solver-input key.
- Do not alter solver formulas or result math, save schema, export/import bytes, Project Plan, or Manual behavior.
...
### Task 2: Add the failing daily solve cache lifecycle contract
...
- [ ] Add cases proving mode isolation, Project requests, `forceFresh`, and malformed cache bytes dispatch a Worker.
- [ ] Run `node test/solve-lifecycle.cjs` and verify the new assertions fail because cache behavior is absent.

### Task 3: Implement the 24-hour bounded Items and Credits cache
...
- Produces: `dailySolveConditionKey(state)`, private read/write/prune helpers, `options.forceFresh`, and callback metadata `{ cached, savedAt }`.

- [ ] Add versioned mode-specific input projections, a collision-safe hash slot, strict record validation, 24-hour expiry, 24-entry pruning, and a 512-KiB record cap.
- [ ] Check the cache for Items and Credits requests before overlay/Worker creation; use existing generation/current-state delivery.
- [ ] Store successful authoritative Items responses and only complete Credits comparisons from both Worker and fallback paths.
