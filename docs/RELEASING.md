...
- `/version.json` is revalidated on the same policy and is one of the two URLs in the release that are deliberately *not* content-addressed: its name is a permanent contract with tabs that are already open. It holds the release id, which is the hash of the generated page before that id is stamped back into it, so it moves whenever a reader would see a different app, stylesheet, or image, and stays put when a deploy changes nothing they load. The same id is stamped into the page's `forge-build` meta; a release where those two disagree fails the build.
- `/boot.js` is the other, revalidated on the same policy, and is the release's answer to a page that is too stale to load itself. Because the alias serves one release at a time, a reader holding an older `index.html` asks for a bundle that is no longer there: the markup paints, nothing runs, and every control is dead. The notice in the app cannot answer that — it ships inside the bundle that failed — so this one small file sits outside the hashed graph, loads ahead of the bundle, and watches for a script or stylesheet that fails to arrive. Only then does it ask `version.json` for the release id, and only an id that disagrees with the page's stamp reloads it, once per stamp per tab. A page whose assets all arrive registers one listener and issues no request.
...
9. Confirm `curl /version.json` returns the same id as the deployed page's `forge-build` meta, and that a second request carrying its `ETag` answers `304`. A tab left open on the previous release then offers its reader a refresh within half an hour of being looked at; a tab that is merely open never reloads on its own.
10. Confirm `curl /boot.js` returns the recovery script under a revalidated cache policy, and that the deployed page loads it ahead of its `static/app...js` bundle. A page that arrives too stale to load that bundle reloads itself once; this is the only case in which the app reloads without being asked.
