...
// Duplication chance (%), global to every line: a base chance from the crafting
// attribute (S.attrDupe) plus a per-turbo-stack bonus (S.trio4, from Trio Affinity 4)
// applied at the global max turbo stacks. dupeMult() is the output multiplier.
function dupeChance(){return Math.max(0,(num(S.attrDupe)||0)+(num(S.maxTurbo)||0)*(num(S.trio4)||0));}
...
function defaults(){
  ...
  return {
    ...
    maxTurbo:0,attrDupe:12.40,trio4:0,
