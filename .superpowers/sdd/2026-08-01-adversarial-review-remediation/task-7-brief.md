...
- The Worker response `__stab` contains only the selected visible policy's records.
...
Never match by display name or array index. Per-phase comparison rows include only phases that were actually stabilized.
...
- both alternatives are executable and every replay boundary is nonnegative.
...
Actions must be truthful:

- `Keep current line jobs`;
- `Use shorter re-optimized plan` only if alternative total ETA is lower;
- otherwise `Use higher-throughput line jobs anyway` when that is true;
...
Remove remaining Project-facing `fastest`/`optimal` promises from the modal, sequence toggle, result summary, and README. Leave accurate Mined-mode `fastest current line` copy unchanged.
...
- Effective zero-gap copy is neutral.
...
- Sequenced/wave phase keys are stable and duplicate display names cannot cross-match.
...
- Selector/help/action relationships are keyboard and screen-reader accessible by construction.
- Generated-app smoke uses the ordinary current Blob Worker: establish schema-v2 prefer-current pins, capture `__stab`, assert the 420 prefer-current full tradeoff and selected held cache, assert 420 reoptimize has no stabilization/hidden comparison and returns the free selected cache, and require Blob transport with zero permanent Worker/dependency requests.
