...
- **Multi-output:** select several outputs at once; the mix numbers shape the shared weighted-output floor. The result names which output is setting that floor and how much room the others have above it, and reports when constraints are infeasible or the bounded search did not finish an exhaustive proof.

- **Two ways to ask for the mix:** **Ratio** asks for an output ratio in item units, so an item that is harder to make costs more to demand at the same number. **Share of max** asks for a percentage of what each output could reach on its own — the planner measures every enabled output's dedicated ceiling and shows it beside the slider, which keeps items whose ceilings differ several-fold comparable on one control. Slider positions do not map one-to-one onto plans: an output's rate is quantized by whole line assignments, so a range of positions can produce the same plan.
