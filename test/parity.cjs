...
(function(){
  ...
  const scenarios = {
    ...
    "items.wire.gel":          s => { s.mode="items"; on(s,["Wire"]); s.gelVesp=GEL_BUDGET; },
    "items.multiWire.gel":     s => { s.mode="items"; on(s,["Wire","Rods"]); s.gelVesp=GEL_BUDGET; },
    "credits.noGel":           s => { s.mode="credits"; prices(s,{Frames:10,Glass:3,Rods:1}); },
    "credits.wire.gel":        s => { s.mode="credits"; prices(s,{Wire:50,Rods:1}); s.gelVesp=GEL_BUDGET; },
    ...
    "project.wire.gel":        s => { s.mode="project"; s.gelVesp=GEL_BUDGET;
      s.projects=[{id:"a",name:"P",catId:"",on:true,from:1,to:1,done:0,prio:null,
