...
(function(){
  ...
  const scenarios = {
    ...
    "items.wire.gel":          s => { s.mode="items"; on(s,["Wire"]); s.minedIncome.Vespium.resourcesTradingPerSec=GEL_BUDGET/60; },
    "items.multiWire.gel":     s => { s.mode="items"; on(s,["Wire","Rods"]); s.minedIncome.Vespium.resourcesTradingPerSec=GEL_BUDGET/60; },
    "credits.noGel":           s => { s.mode="credits"; prices(s,{Frames:10,Glass:3,Rods:1}); },
    "credits.wire.gel":        s => { s.mode="credits"; prices(s,{Wire:50,Rods:1}); s.minedIncome.Vespium.resourcesTradingPerSec=GEL_BUDGET/60; },
    ...
    "project.wire.gel":        s => { s.mode="project"; s.minedIncome.Vespium.resourcesTradingPerSec=GEL_BUDGET/60;
      s.projects=[{id:"a",name:"P",catId:"",on:true,from:1,to:1,done:0,prio:null,
