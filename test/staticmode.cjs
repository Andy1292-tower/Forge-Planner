 ...
 *   "static" — one job per line for the WHOLE phase, never switched, chosen so every demanded
 *              item finishes at about the same time (in demand ratio).
 *
 * What this pins down:
 ...
 *   b) items finish together: per-item finish = net demand / rate, and with a 9:1 demand ratio the
 *      slowest and fastest item land within a discreteness tolerance of each other. This is the
 *      whole point of weighting solveCore by net demand.
...
(function(){
  ...
  // (b) items finish together. Discreteness sets the floor here: a whole line is the smallest unit
  // of capacity, so the ratio can never be exactly 1. 1.5 leaves room for that without letting a
  // genuinely lopsided plan (one item finishing twice as late) pass.
  ...
  record("static: demanded items finish together (ratio <= 1.5)",
